...
import {
    MatTabChangeEvent as MatTabChangeEvent,
    MatTabGroup as MatTabGroup,
...
export class BottomTabsComponent implements OnChanges, OnDestroy {
    ...
    ngOnChanges(changes: SimpleChanges) {
        ...
        if (changes.Tabs && _this.Tabs.length) {
            ...
            changes.SaveData &&
            _this.filteredTabs[_this.activeIndex] != null &&
            _this.filteredTabs[_this.activeIndex].type === "tableForm"
        ) {
    ...
    setFiltered(): void {
        ...
        if (newTabs.length) {
            // check if any visible tab
            _this.filteredTabs = newTabs.sort(
                (a, b) => a.renderingOrder - b.renderingOrder,
            );
            ...
            _this.tableParams = {
                entryName: _this.filteredTabs[_this.activeIndex].table,
                keys: _this.filteredTabs[_this.activeIndex].keys,
                showHeader: true,
                showFullScreenButton: true,
                searchKeys: null,
            };
            _this.formTableParams = {
                entryName: _this.filteredTabs[_this.activeIndex].table,
                keys: _this.filteredTabs[_this.activeIndex].keys,
                showHeader: true,
            };
    ...
    tabChanged(tabChangeEvent: MatTabChangeEvent): void {
        if (
            ...
            if (this.tabsGroup.selectedIndex === tabChangeEvent.index) {
                this.activeIndex =
                    tabChangeEvent.index >= 0 ? tabChangeEvent.index : 0; // might get a -1
                this.tableParams = {
                    entryName: this.filteredTabs[this.activeIndex].table,
                    keys: this.filteredTabs[this.activeIndex].keys,
                    showHeader: true,
                    showFullScreenButton: true,
                    searchKeys: null,
                };
                this.formTableParams = {
                    entryName: this.filteredTabs[this.activeIndex].table,
                    keys: this.filteredTabs[this.activeIndex].keys,
                    showHeader: true,
                };
