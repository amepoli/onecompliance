...
export class DomandeRisposteComponent implements OnChanges
{
    ...
    processData(results: DomandaRispostaElement[]) {
        ...
        for(let i = 0; i < results.length; i++) {
            ...
            const curFormData = _this._formsService.getFormData(_this.viewKeys[i], [newKeys], _this.attributes, _this.formParams[i], results[i].keys, results[i].readonly, 0, _this.isTabMode, _this.isDialog, null)[0].map(x => (x.type === "combobox") ? {...x, value: null}: x);
