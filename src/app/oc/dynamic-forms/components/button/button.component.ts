...
@Component({
    ...
    template: `
        ...
        >
            <button
                *ngIf="
                    field.buttonIcon && !field.label && !field.isDownloadButton
                "
                mat-icon-button
            ...
            <button
                ...
                *ngIf="
                ...
                (click)="onClickButton()"
                [style.width]="'100%'"
                [style.background-color]="field.style.background_color"
            ...
            <button
                *ngIf="!field.buttonIcon && !field.isDownloadButton"
                mat-raised-button
                color="primary"
                [disabled]="field.readonly"
                (click)="onClickButton()"
    ...
    host: {
        "[style.padding-top.px]":
            'field.isVisible? field.buttonIcon || field.isDownloadButton? "8": "13": "8"',
        // '[style.padding-bottom.px]': 'field.isVisible? field.buttonIcon || field.isDownloadButton? "24": "14": "0"',
        "[style.margin-right]":
            'field.isVisible?(field.buttonIcon && !field.label) || field.isDownloadButton? "8px": "0.5%": "0"',
        "[style.margin-left]":
            'field.isVisible? (field.buttonIcon && !field.label) || field.isDownloadButton? "8px": "0.5%": "0"',
        "[style.width]":
            'field.isVisible? (field.buttonIcon && !field.label) || field.isDownloadButton? "32px": field.width + "%": "0"',
        "[style.height.px]":
            'field.isVisible? (field.buttonIcon && !field.label) || field.isDownloadButton? "32": field.buttonIcon? "64": "76": "0"',
        ...
        "[style.margin-bottom.px]":
            'field.isVisible? field.buttonIcon || field.isDownloadButton? "8": "8": "0"',
...
export class ButtonComponent implements OnInit, OnDestroy {
        ...
        // Confirm first if confirmation is true before performing action
            ...
            _this._dialogService
                .showConfimationDialog(
                    _this.field.label ? _this.field.label : _this.field.name,
    ...
    ngOnInit() {
        const _this = this;
        _this.field.style =
            _this.field.style == null
                ? { background_color: "#2A4F9D", font_color: "white" }
                : _this.field.style;
        _this.field.style.background_color =
            _this.field.style.background_color != null
                ? _this.field.style.background_color
                : "#2A4F9D";
        _this.field.style.font_color =
            _this.field.style.font_color != null
                ? _this.field.style.font_color
                : "white";
    }
