    ...
    template: `
        ...
        >
            <button
                ...
                [style.color]="
                    field.style && field.style.font_color
                        ? field.style.font_color
                        : null
                "
                [style.padding]="'0px'"
                [style.border]="'0.1px solid #144FB7'"
...
export class MenuComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if (
            _this.field.menuOptions != null &&
            _this.field.menuOptions.length > 0
        ) {
            _this.filteredMenuOptions = _this.field.menuOptions.filter(
                (x) => !x.isHidden,
            );
        } else {
