...
export class ImportDialogComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    onImport(): void {
        ...
        console.log(event);
        _this.import = false;
        if (_this.files != null && _this.files.length) {
            // get the S3 URL 
            _this.backendService.createImportFileURL(_this.data.entryName, _this.authService.getCurrentCompany(), _this.data.keys).subscribe(
                responseURL => {
                    console.log(responseURL);
                    if (responseURL != null && responseURL.response === 'OK') {
                        responseURL = responseURL.data;
                        const blob = new Blob([_this.files[0]]);
