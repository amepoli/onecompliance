...
<span [formGroup]="group">
  <mat-checkbox *ngIf="field.isVisible != false" [ngStyle]="{'margin-right': '1%', 'margin-left': '1%', 'width': field.width+'%'}" [ngModel]="field.value" [formControlName]="field.name" [disabled]="field.readonly || readOnly" (change)="onCheck($event)">{{field.label}}</mat-checkbox>
</span>`,
  styles: []
})
