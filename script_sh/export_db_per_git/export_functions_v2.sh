...
# Ottieni l'elenco delle functions dallo schema
function_names=$(psql -h "$DB_ENDPOINT" -U "$DB_USER" -d "$DB_NAME" -At -c "
...
for function_name in $function_names; do
    ...
    echo "Esportando function: $function_name in $output_file"
    ...
    if [[ $? -eq 0 ]]; then
        echo "Esportazione completata per $function_name"
    else
        echo "Errore durante l'esportazione di $function_name"
    fi
done

echo "Esportazione completata per tutte le functions."
