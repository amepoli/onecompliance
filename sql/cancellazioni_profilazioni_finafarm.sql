...
ORDER BY 
    ...
    COALESCE(snd.data_esecuzione, snd.data_prevista)
		...
		where snd.codice_azienda='FINAFARM'  
		and snd.codice_azienda=ss.codice_azienda and snd.id_sondaggio=ss.id_sondaggio
		and ss.object_key=snf.object_key
		and snd.id_modello_test=526 
		and snf.rn=1
		and snd.id_sondaggio_succ in(select snf2.id_sondaggio from sondaggi_con_scadenza_nel_futuro snf2 where rn>1)
	),
	aggiorna_somministrazioni as(
		...
		and snd.id_modello_test=526 
		and snf.rn=1
		and ss.id_somministrazione_succ in(select snf2.id_somministrazione from sondaggi_con_scadenza_nel_futuro snf2 where rn>1)
	)
