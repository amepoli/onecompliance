-- FUNCTION: entrasp.risposte_update_insert(character varying, numeric, numeric, text, numeric, numeric, numeric, numeric, numeric, numeric, text, numeric, numeric, character varying, character varying, character varying)

-- DROP FUNCTION IF EXISTS entrasp.risposte_update_insert(character varying, numeric, numeric, text, numeric, numeric, numeric, numeric, numeric, numeric, text, numeric, numeric, character varying, character varying, character varying);
...
CREATE OR REPLACE FUNCTION entrasp.risposte_update_insert(
	...
	rispostamultipla character varying,
...
IF idtipodomanda in(1,3,5,6,7) THEN 
	...
	WHERE codice_azienda = codiceazienda
 	...
 	IF idrisposta IS NULL AND (idrispostaprev IS NOT NULL OR (risposta_ IS NOT NULL AND risposta_ != '')) THEN 
			INSERT INTO entrasp.risposte (codice_azienda, id_modello_test, id_modello_test_vr, id_sondaggio, id_somministrazione, id_domanda, id_risposta_prev, id_risposta, risposta, note, punteggio, peso, penalizzazione)
			VALUES (codiceazienda, idmodellotest, idmodellotestvr, idsondaggio, idsomministrazione, iddomanda, idrispostaprev, nextval('entrasp.id_risposta'), risposta_, note_, punteggio_, coalesce(peso_, peso_oa), coalesce(penalizzazione_, 0)) on conflict do nothing;
			idrisposta := currval('entrasp.id_risposta');
	ELSE 
...
ALTER FUNCTION entrasp.risposte_update_insert(character varying, numeric, numeric, text, numeric, numeric, numeric, numeric, numeric, numeric, text, numeric, numeric, character varying, character varying, character varying)
    OWNER TO postgres;
