...
TABLENAME=`cat ../../${1}.json | jq -r ".dynamoTables.navigation.tableName"`
...
MAINKEY=`cat ../../${1}.json | jq -r ".dynamoTables.navigation.mainKey"`
...
if [ $# -eq 2 ]
  then
    echo "Going to align navigation/"${2}" on "${TABLENAME}
      cd ../../tools/push_tables_s3
      ./run.sh ${TABLENAME} navigation/${2}
    echo "Uploaded to S3!"
  else
