...
if [ $# -eq 2 ]; then
  ...
  # Exclude result.json from the list of files to be processed
  files=($(ls *.json | grep -v '^result.json$')) 
fi
...
for ((i = 0; i < ${#files[@]}; i += n)); do
  (
    echo "["
    for f in "${files[@]:i:n}"; do (
      cat "$f"
      echo ","
    ); done
    echo "]"
  ) | json-dynamo-putrequest --beautify $TABLENAME >dynamo-input/dynamo_"$i".json
done
...
echo "Going to check users's id_anagrafica if duplicate on "${LAMBDANAME}
aws lambda invoke \
