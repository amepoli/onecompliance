...
TABLENAME=`cat ../../${1}.json | jq -r ".dynamoTables.homepages.tableName"`
...
MAINKEY=`cat ../../${1}.json | jq -r ".dynamoTables.homepages.mainKey"`
...
if [ $# -eq 2 ]
  then
    echo "Going to align homepages/"${2}" on "${TABLENAME}
      cd ../../tools/push_tables_s3
      ./run.sh ${TABLENAME} homepages/${2}
    echo "Uploaded to S3!"
  else
