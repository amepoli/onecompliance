...
TABLENAME=`cat ../../${1}.json | jq -r ".dynamoTables.translation.tableName"`
...
MAINKEY=`cat ../../${1}.json | jq -r ".dynamoTables.translation.mainKey"`
...
if [ $# -eq 2 ]
  then
    echo "Going to align translation/"${2}" on "${TABLENAME}
      cd ../../tools/push_tables_s3
      ./run.sh ${TABLENAME} translation/${2}
    echo "Uploaded to S3!"
  else
